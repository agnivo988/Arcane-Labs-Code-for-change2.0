  ...
  useEffect(() => {
    ...
    mm.add(
      ...
      (context) => {
        ...
        const nodes = gsap.utils.toArray<HTMLElement>(".animate-in");
...
}> = ({ theme, onToggle, authProfile }) => {
  const [scrolled, setScrolled] = useState(false);
  const initials = (authProfile?.name || authProfile?.email || "User")
    .trim()
    .charAt(0)
    .toUpperCase();
  useEffect(() => {
    ...
    <header className={`site-nav ${scrolled ? "site-nav-scrolled" : ""}`}>
      <div className="site-nav-inner">
        ...
        <div className="site-right">
          ...
          <NavLink to="/studio" className="cta-main">
            Try Now →
          </NavLink>
        </div>
      </div>
...
const PageHero: React.FC<PageHeroProps> = ({
  ...
  <section className="page-hero animate-in">
    <div
      className={`page-hero-main ${mediaSide === "left" ? "page-hero-main-left" : "page-hero-main-right"}`}
    >
      {media && mediaSide === "left" && (
        <div className="page-hero-media page-hero-media-left">{media}</div>
      )}
      <div className="page-hero-copy">
        ...
        <div className="hero-cta page-hero-cta">
          <NavLink to={primaryAction.to} className="cta-main">
            {primaryAction.label}
          </NavLink>
          {secondaryAction && (
            <NavLink to={secondaryAction.to} className="cta-ghost">
      ...
      {media && mediaSide === "right" && (
        <div className="page-hero-media page-hero-media-right">{media}</div>
      )}
    </div>
...
const heroVideo = (caption: string, source: string, poster?: string) => (
  <HeroMediaPanel caption={caption} source={source} poster={poster} />
);
...
const HomePage: React.FC = () => {
  ...
  const heroBgY = useTransform(
    scrollYProgress,
    [0, 1],
    [0, reduceMotion ? 12 : 180],
  );
  const heroBgScale = useTransform(
    scrollYProgress,
    [0, 1],
    [1.02, reduceMotion ? 1.04 : 1.24],
  );
  const heroBgOpacity = useTransform(scrollYProgress, [0, 1], [0.78, 0.18]);
  const heroCopyY = useTransform(
    scrollYProgress,
    [0, 1],
    [0, reduceMotion ? -8 : -58],
  );
  const heroCopyOpacity = useTransform(scrollYProgress, [0, 1], [1, 0.5]);
  ...
  useEffect(() => {
    ...
    mm.add(
      ...
      (context) => {
        const { reduceMotion: reduced } = context.conditions as {
          reduceMotion: boolean;
        };
        if (reduced) return;
        gsap.to(root.querySelector(".marquee-track"), {
          xPercent: -16,
          ease: "none",
          scrollTrigger: {
            trigger: root.querySelector(".marquee"),
            start: "top bottom",
            end: "bottom top",
            scrub: 1,
          },
  ...
  return (
    <div ref={homeRef} className="page-wrap home-page-wrap">
      <section ref={heroRef} className="hero home-hero home-hero-bleed">
        <motion.div
        ...
        />
        <motion.div
          className="home-hero-cursor-glow"
          style={{ y: heroGlowY }}
          aria-hidden="true"
        />
        ...
        >
          <motion.p
            className="hero-eyebrow animate-in"
            initial={{ opacity: 0, y: 18, filter: "blur(8px)" }}
            animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
            transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          >
            ARcane Labs · March 2026
          </motion.p>
          <h1 className="hero-title">
            <span className="word">Reshape</span>{" "}
            <span className="word">Reality.</span>
            <br />
            <span className="word">In</span> <span className="word">Your</span>{" "}
            <span className="word">Browser.</span>
          </h1>
          <motion.p
            className="hero-sub animate-in"
          ...
          >
            ARcane Engine transforms your live camera feed into an interactive
            canvas — powered by Gemini 2.5 Flash Image.
          </motion.p>
          <motion.div
            className="hero-cta animate-in"
          ...
          >
            ...
            <a
              className="cta-ghost"
              href="https://brown-kelsey-13.tiiny.site"
              target="_blank"
              rel="noreferrer"
            >
              Read Whitepaper
      ...
      <section className="marquee">
        <div className="marquee-track">
          Tap-to-Edit ◆ Text-to-Reality ◆ Fusion Mode ◆ Frame Chaining ◆
          MobileSAM ◆ WebRTC ◆ Sub-2s Generation ◆ Tap-to-Edit ◆ Text-to-Reality
          ◆ Fusion Mode ◆ Frame Chaining ◆ MobileSAM ◆ WebRTC ◆ Sub-2s
          Generation ◆
        </div>
      </section>
      <section className="pipeline-grid">
        {[
          ...
          <article
            key={step}
            className="pipeline-card animate-in"
            style={{ ["--index" as string]: Math.min(i, 4) }}
          >
            ...
            <p>
              {
                [
                  "Capture only what should change while preserving scene intent.",
                  "Choose the exact frame moment where a transformation should begin.",
                  "Separate subject from context so edits remain believable in motion.",
                  "Generate a coherent variation guided by prompt and composition.",
                  "Blend the result into the timeline with stable lighting continuity.",
                  "Repeat small refinements until the narrative frame feels authored.",
                ][i]
              }
            </p>
          </article>
  ...
  return (
    <div className="page-wrap editorial-page">
      <PageHero
        ...
        title={
          <>
            <span>Features built</span> <span>for editable reality.</span>
          </>
        }
        ...
        media={heroVideo(
          "A moving preview of the live Arcane engine surface.",
          "/feature.mp4",
        )}
        mediaSide="right"
      />
      ...
      <section className="page-section">
        ...
        <div className="page-bento-grid">
          <article className="glass-card page-bento page-bento-wide animate-in">
            ...
            <p>
              Pick a region, write the change, and keep the surrounding
              composition coherent.
            </p>
            <div className="page-inline-tags">
              <span>Targeted edits</span>
              <span>Scene-aware output</span>
          ...
          <article className="glass-card page-bento animate-in">
            ...
            <p>
              Generate new imagery that respects the live frame so iteration
              feels immediate.
            </p>
          </article>
          <article className="glass-card page-bento animate-in">
            ...
            <p>
              Combine two inputs into a consistent output with style continuity
      ...
      <section className="marquee page-marquee">
        <div className="marquee-track">
          Tap-to-Edit ◆ Text-to-Reality ◆ Fusion Mode ◆ Live Camera ◆ Prompt
          Chaining ◆ Frame Awareness ◆ Tap-to-Edit ◆ Text-to-Reality ◆ Fusion
          Mode ◆ Live Camera ◆ Prompt Chaining ◆ Frame Awareness ◆
        </div>
      </section>
  ...
  return (
    <div className="page-wrap editorial-page">
      <PageHero
        ...
        title={
          <>
            <span>Technology designed</span>{" "}
            <span>for realtime iteration.</span>
          </>
        }
        ...
        media={heroVideo(
          "The same video panel, anchored on the left to balance the section rhythm.",
          "/tech.mp4",
        )}
        mediaSide="left"
      />
      ...
      <section className="page-section">
        ...
        <div className="page-tech-grid">
          {[
            [
              "Browser Client",
              "React + Vite keeps the UI snappy and lets the studio respond immediately to local input.",
            ],
            [
              "Camera + Capture",
              "The live stream is normalized into frame-ready data before any model call happens.",
            ],
            [
              "Model Routing",
              "Image generation resolves a compatible Gemini model with fallback handling when a preferred model is unavailable.",
            ],
            [
              "Queue + Throttle",
              "Requests are serialized with spacing so edits remain stable under repeated interaction.",
            ],
          ].map(([title, copy], index) => (
            <article
              key={title}
              className="glass-card page-tech-card animate-in"
              style={{ ["--index" as string]: index }}
            >
      ...
      <section className="page-section">
        ...
        <div className="pipeline-grid">
          {[
            ...
            [
              "Select",
              "Choose the area or reference the model should respect.",
            ],
            [
              "Transform",
              "Send the prompt through the routed Gemini image model.",
            ],
            ["Render", "Bring the returned image back into the canvas."],
            [
              "Repeat",
              "Keep refining while the visual context remains intact.",
            ],
          ].map((step, index) => (
            <article
              key={step[0]}
  ...
  return (
    <div className="page-wrap editorial-page">
      <PageHero
        ...
        title={
          <>
            <span>Built for teams</span> <span>that think in scenes.</span>
          </>
        }
        ...
        media={heroVideo(
          "A cinematic cue that signals motion, transformation, and iteration.",
          "/use_case.mp4",
        )}
        mediaSide="right"
      />
      ...
      <section className="page-section">
        ...
        <div className="page-usecase-grid">
          {[
            [
              "Creative direction",
              "Iterate on concepts, variants, and mood shifts while keeping a coherent visual thread.",
            ],
            [
              "Product and marketing",
              "Produce campaign visuals, landing mockups, and compositional studies with tight feedback loops.",
            ],
            [
              "Architecture and interiors",
              "Move from reference imagery to localized changes without rebuilding the whole scene.",
            ],
            [
              "Education and demos",
              "Show how prompt-driven image workflows work in a way that feels tangible and memorable.",
            ],
          ].map((item, index) => (
            <article
              key={item[0]}
              className="glass-card page-usecase-card animate-in"
              style={{ ["--index" as string]: index }}
            >
              <p className="page-card-kicker">0{index + 1}</p>
              <h3>{item[0]}</h3>
      ...
      <section className="page-section">
        <div className="glass-card page-quote animate-in">
          ...
          <blockquote>
            Keep the creation loop visible. The UI should make it obvious what
            changes, why it changes, and how quickly the result can be refined.
          </blockquote>
        </div>
      </section>
    </div>
...
const AboutPage: React.FC = () => (
  <div className="page-wrap about-wrap">
    <h2>ARcane Labs</h2>
    <p>
      ARcane Engine represents more than an incremental improvement in AR
      technology — it marks a fundamental shift in how humans interact with and
      reshape their visual reality.
    </p>
    <a href="https://github.com/agnivo988" target="_blank" rel="noreferrer">
      $ github.com/agnivo988
    </a>
    <a href="https://github.com/Priyangshu-7" target="_blank" rel="noreferrer">
      $ github.com/Priyangshu-7
    </a>
  </div>
);
  ...
  return (
    <div className="page-wrap editorial-page profile-page-wrap">
      <PageHero
        ...
        title={
          <>
            <span>Your Arcane</span> <span>usage dashboard.</span>
          </>
        }
      ...
      <section className="page-section profile-stack">
        <article className="glass-card profile-identity animate-in">
          ...
          <div className="profile-actions">
            ...
            <button className="cta-main" onClick={onLogout} type="button">
              Log out
            </button>
          </div>
        </article>
          ...
          {[
            {
              ...
              value: profile?.promptsGenerated ?? 0,
            },
            ...
            <article
              key={item.label}
              className="glass-card profile-metric animate-in"
            >
        ...
        <article className="glass-card profile-details animate-in">
          ...
          <div className="profile-detail-list">
            <p>
              ...
              {profile?.workspaceName || "Main Studio"}
            </p>
            <p>
              <strong>Plan:</strong> {profile?.plan || "creator"}
            </p>
            <p>
    ...
    <div className="page-wrap editorial-page">
      <PageHero
        ...
        title={
          initialMode === "signup" ? (
            <>
              Create your <span>ARcane Studio.</span>
            </>
          ) : (
            <>
              Sign into <span>your workspace.</span>
            </>
          )
        }
        description={
        ...
        primaryAction={{
          label: initialMode === "signup" ? "Get Started" : "Continue",
          to: "#",
        }}
        stats={
          initialMode === "signup"
            ? ["Free Account", "0 Installs", "Unlimited Creativity"]
            : ["Secure Login", "Usage Tracking", "Your Workspace"]
        }
      />
      <AuthSection
...
const SiteChrome: React.FC<{
  ...
  onToggleTheme: () => void;
  authProfile: UserProfile | null;
}> = ({ children, theme, onToggleTheme, authProfile }) => (
  <>
    <div className="glare-layer">
      <div className="glare-beam" />
    </div>
    <Navbar theme={theme} onToggle={onToggleTheme} authProfile={authProfile} />
    <main className="site-content">{children}</main>
    <footer className="site-footer">
      <p>ARcane Labs · 2026 · Reality is editable.</p>
    </footer>
  </>
);

const AppRoutes: React.FC<{ theme: ThemeMode; onToggleTheme: () => void }> = ({
  theme,
  onToggleTheme,
}) => {
  ...
  const [authToken, setAuthToken] = useState<string | null>(() =>
    localStorage.getItem("arcane-auth-token"),
  );
  const [authProfile, setAuthProfile] = useState<UserProfile | null>(null);
  const [authLoading, setAuthLoading] = useState(false);
  const [hasHydratedServerSession, setHasHydratedServerSession] =
    useState(false);
  ...
  const { generateImage, editImage, fuseImages, isLoading, error, clearError } =
    useGeminiImage(apiKey);
  const historyLookup = useMemo(
    () => new Map(imageHistory.map((image) => [image.id, image])),
    [imageHistory],
  );
  const compareLeft = compareSelection.left
    ? historyLookup.get(compareSelection.left) || null
    : null;
  const compareRight = compareSelection.right
    ? historyLookup.get(compareSelection.right) || null
    : null;
  ...
  const visibleHistory = imageHistory.length
    ? imageHistory
    : currentImage
      ? [currentImage]
      : [];
  ...
  const requiresUpgrade =
    !!authToken && !isProPlan && imagesGeneratedCount >= FREE_IMAGE_LIMIT;

  const toGeneratedImage = (item: {
    id: string;
    prompt: string;
    timestamp: string;
    base64?: string;
  ...
  const extractCollabCode = (input: string) => {
    ...
    try {
      ...
      const collabIdx = parts.findIndex(
        (part) => part.toLowerCase() === "collab",
      );
  ...
  const applyIncomingSession = (session: ServerSessionState) => {
    const sourceHistory =
      Array.isArray(session.imageHistory) && session.imageHistory.length > 0
    ...
    setImageHistory((prev) => {
      const incomingById = new Map(
        incomingHistory.map((item) => [item.id, item]),
      );
      const merged = [
        ...incomingHistory,
    ...
    if (session.lastActivity?.id && session.lastActivity?.text) {
      setCollabActivities((prev) =>
        [
          session.lastActivity as CollaborationActivity,
          ...prev.filter((item) => item.id !== session.lastActivity?.id),
        ].slice(0, 30),
      );
    }
  };
      ...
      session: {
        currentImage: currentImage
          ? {
              ...
              ...(currentImage.base64
                ? { base64: currentImage.base64.slice(0, 1_200_000) }
                : {}),
            }
          : null,
  ...
  const importCollaborationCode = async (input: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          response.status === 404
            ? "Collaboration link not found."
            : "Could not import collaboration link.",
        );
      }
      const data = (await response.json()) as CollaborationLoadResponse;
      if (!data.session)
        throw new Error("This collaboration link has no session data.");
      ...
      showToast(
        err instanceof Error
          ? err.message
          : "Failed to import collaboration link.",
        "error",
      );
    } finally {
  ...
  const createCollaborationLink = async () => {
    ...
    try {
      ...
      const includeCurrentBase64 =
        currentBase64.length > 0 && currentBase64.length <= 1_200_000;
      ...
      showToast(
        err instanceof Error
          ? err.message
          : "Failed to create collaboration link.",
        "error",
      );
    } finally {
  ...
  const assertWithinGenerationLimit = () => {
    ...
    showToast(
      "Free plan limit reached. Upgrade to Pro for more generations.",
      "error",
    );
    return false;
  };
  ...
  useEffect(() => {
    try {
      ...
      if (snapshot.currentImage)
        setCurrentImage(fromStoredImage(snapshot.currentImage));
      if (Array.isArray(snapshot.imageHistory)) {
        setImageHistory(
          snapshot.imageHistory.slice(0, 24).map(fromStoredImage),
        );
      }
      if (snapshot.compareSelection)
        setCompareSelection(snapshot.compareSelection);
    } catch {
      ...
      try {
        ...
        setImageHistory((prev) => {
          ...
          const hasServerHistory =
            Array.isArray(serverSession.imageHistory) &&
            serverSession.imageHistory.length > 0;
          ...
          const merged = [
            ...
            ...prev.filter(
              (item) =>
                !serverHistory.some((serverItem) => serverItem.id === item.id),
            ),
          ].slice(0, 24);
        ...
        if (serverSession.currentImage) {
          ...
          setCurrentImage((prev) => {
            ...
            if (prev && prev.base64 && prev.id !== serverCurrent.id)
              return prev;
            return {
              id: serverCurrent.id,
              prompt: serverCurrent.prompt,
              base64:
                typeof serverCurrent.base64 === "string"
                  ? serverCurrent.base64
                  : "",
  ...
  useEffect(() => {
    ...
    const code =
      codeFromQuery && /^[A-Za-z0-9]{6,12}$/.test(codeFromQuery)
  ...
  useEffect(() => {
    ...
    const socket = io(COLLAB_WS_ORIGIN, {
    ...
    socket.on(
      "collab:session",
      (payload: { code: string; session: ServerSessionState }) => {
        if (!payload?.session) return;
        if (
          payload.code &&
          payload.code.toUpperCase() !== activeCollabCode.toUpperCase()
        )
          return;
    ...
    socket.on("collab:error", (payload: { message?: string }) => {
      setCollabStatus("idle");
      showToast(
        payload?.message || "Collaboration connection failed.",
        "error",
      );
    });
  ...
  useEffect(() => {
    const syncToken = () =>
      setAuthToken(localStorage.getItem("arcane-auth-token"));
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("arcane-profile-refresh", handleUsageRefresh);
  }, [authToken]);
    ...
    void (async () => {
      if (status === "success" && sessionId && authToken) {
        try {
          ...
          if (!response.ok) {
            throw new Error(
              data?.message || "Could not verify checkout result.",
            );
          }

          showToast(
            "Pro upgraded successfully. Unlimited generation unlocked.",
            "success",
          );
  ...
  const handleGenerate = async (prompt: string) => {
    ...
    try {
      ...
      setImageHistory((prev) =>
        [nextImage, ...prev.filter((item) => item.id !== nextImage.id)].slice(
          0,
          24,
        ),
      );
  ...
  const handleEdit = async (prompt: string) => {
    if (!assertWithinGenerationLimit()) return;
    if (!currentImage)
      return showToast("No image to edit. Generate an image first.", "error");
    try {
  ...
  const handleFuse = async (prompt: string) => {
    ...
    if (!uploadedImages.img1 || !uploadedImages.img2)
      return showToast("Please upload two images to fuse.", "error");
    try {
      const base64 = await fuseImages(
        uploadedImages.img1,
        uploadedImages.img2,
        prompt,
      );
      ...
      showToast(
        err instanceof Error ? err.message : "Failed to fuse images",
        "error",
      );
    }
  };
    ...
    () => (
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          disabled={!apiKey}
          className="arcane-btn arcane-btn-primary disabled:opacity-45 disabled:cursor-not-allowed"
        >
          Enter Live
        </button>
      </div>
  ...
  const CollaborationHubPage: React.FC = () => (
    ...
    >
      <motion.section
        className="min-h-[70vh] p-6 sm:p-8"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      >
        <div className="max-w-3xl mx-auto glass-panel rounded-3xl p-6 sm:p-8 space-y-5">
          ...
          <p className="text-slate-300">
            Create or join a room, then both collaborators can see studio
            activities live.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-2">
            ...
            <button
              ...
              type="button"
              onClick={() => void importCollaborationCode(shareInput)}
              disabled={!shareInput.trim() || isImportingShare}
            >
              {isImportingShare ? "Joining..." : "Join"}
            </button>
          </div>
          {shareLink && (
            <p className="text-xs text-cyan-200/85 break-all">
          ...
          {activeCollabCode && (
            <div className="flex flex-wrap gap-3 items-center">
              <p className="text-sm text-slate-200">
                Room: <span className="text-cyan-200">{activeCollabCode}</span>
              </p>
              <button
                className="arcane-btn arcane-btn-primary"
                type="button"
                onClick={() => navigate(`/studio?collab=${activeCollabCode}`)}
              >
  ...
  const CollaborationLinkPage: React.FC = () => {
    ...
    const [linkData, setLinkData] = useState<CollaborationLoadResponse | null>(
      null,
    );
      ...
      const load = async () => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              response.status === 404
                ? "This collaboration link does not exist."
                : "Could not load this collaboration link.",
            );
          }
          const data = (await response.json()) as CollaborationLoadResponse;
          if (!cancelled) {
      ...
      >
        ...
        >
          <div className="max-w-3xl mx-auto glass-panel rounded-3xl p-6 sm:p-8">
            <p className="text-xs uppercase tracking-[0.22em] text-cyan-300/75 mb-2">
              Collaboration Link
            </p>
            <h1 className="brand-title text-3xl sm:text-4xl mb-3">
              Shared ARcane Session
            </h1>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                <p className="text-slate-200">
                  Code: <span className="text-cyan-200">{linkData.code}</span>
                </p>
                ...
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setShareLink(
                        `${window.location.origin}/collab/${linkData.code.toUpperCase()}`,
                      );
        ...
        <Routes location={location} key={location.pathname}>
          ...
          <Route
            ...
            element={
              ...
              >
                <motion.div
                  className="min-h-screen app-surface text-slate-100 p-4 sm:p-6 lg:p-8"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                >
                  <div className="max-w-7xl mx-auto">
                    <header className="mb-8">
                      <motion.div
                        className="glass-panel rounded-3xl px-5 py-5 sm:px-7 sm:py-6"
                        initial={{ opacity: 0, y: 14 }}
                        animate={{ opacity: 1, y: 0 }}
                      >
                        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <p className="text-xs uppercase tracking-[0.22em] text-cyan-300/75">
                              Realtime Creative Studio
                            </p>
                            <h1 className="brand-title text-4xl sm:text-5xl">
                              ARcane Engine
                            </h1>
                            <p className="text-slate-300 mt-2">
                              Fast visual iteration for camera-based world
                              transformation.
                            </p>
                          </div>
                          {studioActions}
                    ...
                    <div className="flow-strip mb-6">
                      {["Camera", "Select", "Mask", "Generate", "Blend"].map(
                        (item, idx) => (
                          <span key={item} className="flow-pill">
                            {idx + 1}. {item}
                          </span>
                        ),
                      )}
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mt-8">
                      <div className="space-y-6">
                        <PromptForm
                          ...
                          hasUploadedImages={
                            !!(uploadedImages.img1 && uploadedImages.img2)
                          }
                        />
                      ...
                      <div className="space-y-6">
                          ...
                          {!visibleHistory.length ? (
                            ...
                            <div className="space-y-3 max-h-[340px] overflow-auto pr-1">
                              {visibleHistory.map((item) => (
                                <div
                                  key={item.id}
                                  className="flex items-center gap-3 border border-white/10 rounded-xl p-2"
                                >
                                  {item.base64 ? (
                                    <img
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        emitCollabActivity("set compare A");
                                      }}
                                    >
                                      A
                                    </button>
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        emitCollabActivity("set compare B");
                                      }}
                                    >
                                      B
                                    </button>
                            ...
                            {!collabActivities.length ? (
                              ...
                              <div className="space-y-2 max-h-52 overflow-auto pr-1">
                                {collabActivities.map((activity) => (
                                  <div
                                    key={activity.id}
                                    className="border border-white/10 rounded-xl p-2 bg-black/20"
                                  >
                        ...
                        <div className="glass-panel rounded-2xl p-6">
                          ...
                          {isCompareReady && compareLeft && compareRight ? (
                            ...
                            <p className="text-sm text-slate-300">
                              Pick two different history entries using A and B
                              to compare outputs.
                            </p>
                          )}
                        </div>
        ...
        <div className="fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm p-4 grid place-items-center">
          <div className="w-full max-w-md glass-panel rounded-3xl p-6 border border-cyan-200/30">
            ...
            <p className="text-sm text-slate-300 mb-5">
              You have used {imagesGeneratedCount}/{FREE_IMAGE_LIMIT} free
              generations. Upgrade to Pro for more image generation access.
            </p>
...
const App: React.FC = () => {
  const [theme, setTheme] = useState<ThemeMode>(
    () => (localStorage.getItem("arcane-theme") as ThemeMode) || "system",
  );
  ...
  useEffect(() => {
    ...
    raf = window.requestAnimationFrame(tick);
    const done = window.setTimeout(() => setLoaderDone(true), 2400);
    const close = window.setTimeout(() => setIsBooting(false), 2780);
    return () => {
  ...
  return (
    <>
      ...
      {isBooting ? (
        <ImmersiveLoader done={loaderDone} progress={loaderProgress} />
      ) : (
        <AppRoutes
          theme={theme}
          onToggleTheme={() =>
